...
public class TecnicoVeterinarioDAO extends GenericDAO<TecnicoVeterinario> {
    ...
    public boolean eliminar(int id) throws SQLException {
        ...
        try {
            ...
            if ("ELIMINADO_EXITOSAMENTE".equals(resultado)) {
                ...
                throw new SQLException("No se puede eliminar: El técnico tiene tomas de muestra asociadas");
            } else if ("TECNICO_NO_EXISTE".equals(resultado)) {
                throw new SQLException("El técnico no existe");
            }
    ...
    public String verificarDependencias(int id) throws SQLException {
        String sql = "{CALL sp_verificar_dependencias_tecnico(?)}";
        CallableStatement cs = null;
        ResultSet rs = null;
        ...
        try {
            ...
            if (rs.next()) {
                int tomasMuestra = rs.getInt("tomas_muestra");

                if (tomasMuestra > 0) {
                    return "El técnico tiene:\n• " + tomasMuestra + " toma(s) de muestra registrada(s)\n";
                }
            }
            return null;
        } finally {
